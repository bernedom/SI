...
constexpr auto operator*(const electric_current_t<_ratio_lhs> &lhs,
  ...
  return unit_cast<electric_charge_t<_ratio_lhs>>(
      electric_charge_t<std::ratio_multiply<_ratio_lhs, _ratio_rhs>>{
...
constexpr auto operator*(const time_t<1, _ratio_lhs> &lhs,
  ...
  static_assert(detail::is_ratio<_ratio_lhs>::value &&
                    detail::is_ratio<_ratio_rhs>::value,
                "template parametes are ratios");
  return unit_cast<electric_charge_t<_ratio_lhs>>(rhs * lhs);
}
