...
TEMPLATE_TEST_CASE("GIVEN to values of different type AND ratio is the same "
  ...
  constexpr unit_t<'Y', std::ratio<1>, TestType, std::ratio<1>> v2{1};
...
TEMPLATE_TEST_CASE(
    ...
    "[unit_t][helpers]", int64_t, long double) {
  constexpr unit_t<'X', std::ratio<1>, TestType, std::milli> v1{1};
  constexpr unit_t<'Y', std::ratio<1>, TestType, std::kilo> v2{1};
...
TEMPLATE_TEST_CASE(
  ...
  using common_unit = typename SI::detail::unit_with_common_ratio<
      unit_t<'X', std::ratio<1>, TestType, std::ratio<1>>,
      unit_t<'X', std::ratio<1>, TestType, std::milli>>::type;

  STATIC_REQUIRE(
      std::ratio_equal<typename common_unit::ratio, std::milli>::value);
  STATIC_REQUIRE(
      std::is_same<common_unit, SI::detail::unit_t<'X', std::ratio<1>, TestType,
                                                   std::milli>>::value);
}
...
TEST_CASE("GIVEN two values AND ratio is 1 WHEN multiplied with each other AND "
  ...
  constexpr unit_t<'X', std::ratio<1>, int64_t, std::ratio<1>> v2{1};
  constexpr unit_t<'Y', std::ratio<1>, int64_t, std::ratio<1>> v3{1};
  ...
  STATIC_REQUIRE(std::ratio_less<decltype(result_func)::exponent,
