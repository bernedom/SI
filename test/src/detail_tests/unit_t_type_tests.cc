...
TEST_CASE("GIVEN a variable of type unit_t WHEN passed to is_unit THEN result "
          "is true") {
  constexpr unit_t<'X', std::ratio<1>, int64_t> v1{0};
...
TEST_CASE(
  ...
  constexpr unit_t<'X', std::ratio<1>, long double, std::ratio<1>> v1{1};
  constexpr auto result =
      unit_cast<unit_t<'X', std::ratio<1>, long double, std::kilo>>(v1);
  constexpr unit_t<'X', std::ratio<1>, long double, std::kilo> expected(0.001);
...
TEMPLATE_TEST_CASE(
  ...
  constexpr auto result =
      unit_cast<unit_t<'X', std::ratio<1>, TestType, std::ratio<1>>>(v1);
...
TEMPLATE_TEST_CASE(
  ...
  constexpr unit_t<'X', std::ratio<1>, TestType, std::milli> v2{1};
  ...
  STATIC_REQUIRE(
      std::is_same<result_type,
                   unit_t<'X', std::ratio<1>, TestType, std::milli>>::value);
}
