...
TEST_CASE("GIVEN values with ratio 1 THEN value holders returns "
          "ratio<1>")
{
    const SI::unit_t<'X', 1, std::ratio<1>> v1{0};
    const SI::unit_t<'X', 1, std::ratio<1>> v2{0};
...
TEST_CASE(
    ...
    "ratio<1, 1000>")
{
    constexpr SI::unit_t<'X', 1, std::ratio<1>> v1{0};
    constexpr SI::unit_t<'X', 1, std::milli> v2{0};
...
{
    constexpr SI::unit_t<'X', 1, std::ratio<1>> v1{0};
    constexpr SI::unit_t<'X', 1, std::kilo> v2{0};
...
TEST_CASE("GIVEN values with ratio<1000,1> and <1, 1000> THEN value "
          ...
          "ratio<1000000, 1>")
{
    constexpr SI::unit_t<'X', 1, std::milli> v1{0};
    constexpr SI::unit_t<'X', 1, std::kilo> v2{0};
...
{
    constexpr SI::unit_t<'X', 1, std::milli> v1{0};
    constexpr SI::unit_t<'X', 1, std::deci> v2{0};
...
TEST_CASE("GIVEN values with ratio<1000, 1> and <100, 1> THEN value holder "
          "returns ratio<100,1>")
{
    constexpr SI::unit_t<'X', 1, std::kilo> v1{0};
    constexpr SI::unit_t<'X', 1, std::hecto> v2{0};
    constexpr auto rr =
        SI::detail::ratio_to<decltype(v1)::ratio, decltype(v2)::ratio>();
