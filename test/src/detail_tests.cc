...
TEST_CASE("GIVEN values with ratio 1 THEN value holders returns "
          "ratio<1>")
{
  const SI::value_holder_t<'X', std::ratio<1>> v1{0};
  const SI::value_holder_t<'X', std::ratio<1>> v2{0};
...
TEST_CASE(
    ...
    "ratio<1, 1000>")
{
  constexpr SI::value_holder_t<'X', std::ratio<1>> v1{0};
  constexpr SI::value_holder_t<'X', std::milli> v2{0};
...
{
  constexpr SI::value_holder_t<'X', std::ratio<1>> v1{0};
  constexpr SI::value_holder_t<'X', std::kilo> v2{0};
...
TEST_CASE("GIVEN values with ratio<1000,1> and <1, 1000> THEN value "
          ...
          "ratio<1000000, 1>")
{
  constexpr SI::value_holder_t<'X', std::milli> v1{0};
  constexpr SI::value_holder_t<'X', std::kilo> v2{0};
...
{
  constexpr SI::value_holder_t<'X', std::milli> v1{0};
  constexpr SI::value_holder_t<'X', std::deci> v2{0};
...
TEST_CASE("GIVEN values with ratio<1000, 1> and <100, 1> THEN value holder "
          "returns ratio<100,1>")
{
  constexpr SI::value_holder_t<'X', std::kilo> v1{0};
  constexpr SI::value_holder_t<'X', std::hecto> v2{0};
  constexpr auto rr = SI::detail::ratio_to<decltype(v1)::ratio, decltype(v2)::ratio>();
